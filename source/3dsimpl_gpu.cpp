...
void gpu3dsDrawLayers(SLayerList *list) {
    ...
    // Mosaic state is consumed directly by gfxhw.cpp's per-block vertex
    // emit (Phase 2C). No top-level gate is needed in the draw loop.
    ...
    if (list->anythingOnSub) {
        ...
        for (int j = 0; j < list->layersTotalByTarget[TARGET_SNES_SUB]; j++) {
            ...
            // Sub-screen mosaic is rare and skipped (matches mosaic-2c v0).
            // Mosaic for main-screen BGs is emitted as per-block vertices in
            // gfxhw.cpp's S9xDrawBackgroundMosaicHardware* path, so neither
            // this loop nor the per-eye main loop below need a special gate.
